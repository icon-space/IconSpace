...
export class ReactRuntimeGenerator extends RuntimeGenerator {
    protected processPlatformImports(): void {
        if (this.useType) {
            this.writeLine('import React, {HTMLAttributes, ReactElement, createContext,' + " useContext, useMemo} from 'react';")
        } else {
    ...
    protected processPlatformTypes(): void {
        ...
        this.writeLine(`export interface ${this.getInterfaceName('')} extends ${this.getInterfaceName('base')} {`)
        this.indent(1)
        ...
        this.writeLine(`export type ${this.getInterfaceName('props')} = ` + `Intersection<${this.getInterfaceName('')}, HTMLAttributes<HTMLSpanElement>>;`)

        this.writeLine()
        this.writeLine('// 包裹前的图标')
        this.writeLine(`export type ${this.getTypeName('Render')} = ` + `(props: ${this.getInterfaceName('props', true)}) => ReactElement;`)

        this.writeLine()
        this.writeLine('// 包裹后的图标')
        this.writeLine(`export type ${this.getTypeName('')} = (props: ${this.getInterfaceName('props')}) => ReactElement;`)
    ...
    protected processPlatformCode(): void {
        ...
        this.writeLine(`const ${contextName} ` + `= createContext(DEFAULT_${this.prefix.toUpperCase()}_CONFIGS);`)
        this.writeLine()
