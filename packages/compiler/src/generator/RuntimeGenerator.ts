...
export abstract class RuntimeGenerator extends Generator {
    ...
    protected processConstTypes(): void {
        const { theme, hueList, strokeLinejoin, strokeLinecap } = this
    ...
    protected processConfigData(): void {
        ...
        this.writeLine('// 默认属性')
        this.writeLine(`export const DEFAULT_${this.prefix.toUpperCase()}_CONFIGS` + (useType ? `: ${this.getInterfaceName('config')}` : '') + ' = {')
        this.indent(1)
                        ...
                        item.fill
                            ...
                            .forEach((f, i, arr) => {
                                ...
                                if (f.type === 'hue') {
                                    ...
                                    this.writeLine(`${camelCase(name)}: '${f.color}'` + (i !== arr.length - 1 ? ',' : ''))
                                }
                            })
                        this.indent(-1)
                        this.writeLine(`}${i !== arr.length - 1 ? ',' : ''}`)
    ...
    protected processUtil(): void {
        ...
        this.indent(1)
        this.writeLine(`return '${this.prefix}-' + (((1 + Math.random()) * 0x100000000) | 0).toString(16).substring(1);`)
        this.indent(-1)
        this.writeLine('}')
        this.writeLine()
    }
    ...
    protected processConverter(): void {
        ...
        if (this.useType) {
            this.writeLine(
                `export function ${pascalCase(this.prefix)}Converter` +
    ...
    protected processWrapper(): void {
        ...
        this.write(`export function ${this.getTypeName('wrapper')}(`)
