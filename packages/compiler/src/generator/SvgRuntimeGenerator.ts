...
export class SvgRuntimeGenerator extends RuntimeGenerator {
    ...
    protected processPlatformTypes(): void {
        ...
        this.writeLine(`export type ${this.getTypeName('Render')} = ` + `(props: ${this.getInterfaceName('props', true)}) => string;`)

        this.writeLine()
        this.writeLine('// 包裹后的图标')
        this.writeLine(`export type ${this.getTypeName('')} = (props: ${this.getInterfaceName('props')}) => string;`)
