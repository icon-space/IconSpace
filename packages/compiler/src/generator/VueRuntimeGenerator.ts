...
export class VueRuntimeGenerator extends RuntimeGenerator {
    protected processPlatformImports(): void {
        if (this.useType) {
            ...
            this.writeLine('import {ArrayPropsDefinition, DefaultComputed, DefaultData, DefaultMethods} ' + "from 'vue/types/options';")
        } else {
    ...
    protected processPlatformTypes(): void {
        ...
        this.writeLine(`export interface ${instance} extends Vue, ${this.getInterfaceName('props')} {`)
        this.indent(1)
        ...
        this.writeLine(`export type ${this.getTypeName('Render')} = ` + `(h: ${this.getTypeName('helper')}, props: ${this.getInterfaceName('props', true)}) => VNode;`)

        this.writeLine()
        this.writeLine('// 包裹后的图标')
        this.writeLine(`export type ${this.getTypeName('')} = VueConstructor<${instance}>;`)
