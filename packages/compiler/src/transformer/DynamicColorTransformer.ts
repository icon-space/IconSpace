...
function dynamicColorTransformer<T extends ISvgAttr | ISvgStyleAttr>(attr: T, options: IDynamicColorTransformerOptions, ignore: boolean = false): T | null {
    const prefix = options.prefix || false
    const colors: Array<{ color: string; prop: string } | string> = options.array
    ...
    if (name === 'stroke' || name === 'fill') {
        const color = colors.find(item => isColorEqual(isString(item) ? item : item.color, expression))

        if (color != null) {
            return {
                ...attr,
                type: SvgShapeAttr.DYNAMIC,
                expression: (prefix ? (options.propName || 'props') + '.' : '') + (isString(color) ? '' : color.prop)
            }
        }
    }
